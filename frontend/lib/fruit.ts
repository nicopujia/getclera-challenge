/**
 * Fruit Generation & Communication
 *
 * Client-side port of supabase/functions/_shared/generateFruit.ts
 * Pure JS — no Deno or server dependencies.
 */
  ...
  return {
    ...
    hasStem: randomChance(NULL_PROBABILITY)
      ...
      : type === "apple"
        ? randomChance(0.7)
        : randomChance(0.1),
    hasLeaf: randomChance(NULL_PROBABILITY) ? null : randomChance(0.25),
    hasWorm: randomChance(NULL_PROBABILITY) ? null : randomChance(0.08),
    ...
    hasChemicals: randomChance(NULL_PROBABILITY) ? null : randomChance(0.35),
  };
}
...
function generatePreferences(attributes: FruitAttributes): FruitPreferences {
  ...
  if (randomChance(0.25)) {
    preferences.hasStem = randomChance(0.6);
  }
  ...
  if (randomChance(0.45)) {
    const numAcceptable = randomPick([1, 2, 2, 3]);
    if (numAcceptable === 1) {
  ...
  if (randomChance(0.4)) {
    preferences.hasChemicals = randomChance(0.2);
  }
...
export function communicateAttributes(fruit: Fruit): string {
  ...
  if (attributes.hasStem !== null) {
    const connector = randomPick(CONNECTORS);
    const stemDesc = randomPick(
      STEM_TEMPLATES[attributes.hasStem.toString() as "true" | "false"]
    );
    parts.push(`${connector} ${stemDesc}.`.trim());
  }

  if (
    attributes.hasLeaf !== null &&
    (attributes.hasLeaf || randomChance(0.5))
  ) {
    const connector = randomPick(CONNECTORS);
    const leafDesc = randomPick(
      LEAF_TEMPLATES[attributes.hasLeaf.toString() as "true" | "false"]
    );
  ...
  if (attributes.hasWorm !== null) {
    if (attributes.hasWorm || randomChance(0.4)) {
      const connector = randomPick(CONNECTORS);
      const wormDesc = randomPick(
        WORM_TEMPLATES[attributes.hasWorm.toString() as "true" | "false"]
      );
  ...
  if (attributes.hasChemicals !== null && randomChance(0.7)) {
    const connector = randomPick(CONNECTORS);
    const chemDesc = randomPick(
      CHEMICALS_TEMPLATES[
        attributes.hasChemicals.toString() as "true" | "false"
      ]
    );
...
export function generateFruit(type: FruitType): Fruit {
  ...
  return { type, attributes, preferences };
}
